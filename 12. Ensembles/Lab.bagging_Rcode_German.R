###############################################
#
# Lab: Bagging with German Credit Data 
#
###############################################
...
fit = bagging(y~., data=german, mfinal=50, control=my.control)
...
pred2 = predict.bagging(fit, newdata=german)$prob
...
set.seed(1234)
my.control = rpart.control(xval=0, cp=0, minsplit=5, maxdepth=10)
fit = bagging(y~., data=german.train, mfinal=50, control=my.control)
...
pred = predict.bagging(fit, newdata=german.test)
cutoff = 0.5
yhat = ifelse(pred$prob[,2] > cutoff, 1, 0)
...
pred2 = predict.bagging(fit, newdata=german.train)$prob
...
pred2 = predict.bagging(fit, newdata=german.test)$prob
